...
class Trnl(object):
    ...
    if not os.path.isdir(gc_dl_dir):
        ...
        subprocess.run(['sudo', 'add-apt-repository', 'universe'],shell=False)
        subprocess.run(['sudo', 'apt-get', 'update'],shell=False)
        subprocess.run(['sudo', 'apt-get', 'install', 'libgconf2-4', 'libnss3-1d', 'libxss1'],shell=False)
        subprocess.run(['sudo', 'dpkg', '-i', gc_dl_dir + '/google-chrome-stable_105.0.5195.125-1_amd64.deb'],shell=False)
        subprocess.run(['sudo', 'apt-get', 'install', '-f'],shell=False)
